...
export class MemoryManager {
  ...
  addRun(synthesizedData) {
    const { delta } = this.prepareDelta(synthesizedData);
    this.persist(synthesizedData, delta);
    return delta;
  }

  /**
   * Computes the delta against the TRUE previous run and reads that run's ideas.
   * Reads getLastRun() before any mutation — mutates nothing, so it is safe to
   * call before the current sweep's ideas have been assigned. Split out of
   * addRun() so callers can compute the delta first, finalize ideas using
   * previousIdeas, and only then persist (see persist()).
   */
  prepareDelta(synthesizedData) {
    const previous = this.getLastRun();
    const previousIdeas = previous?.ideas || [];
    ...
    return { delta, previousIdeas };
  }

  /**
   * Persists a fully-formed run (ideas already assigned) using an
   * already-computed delta (from prepareDelta()).
   */
  persist(synthesizedData, delta) {
